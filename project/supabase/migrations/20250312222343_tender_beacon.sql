/*
  ...
  1. New Tables
    ...
    - match_history: Store game activity
      - id (uuid)
      - user_id (uuid)
      - match_data (jsonb)
  ...
  2. Security
    ...
    - Add policies for match history
*/
...
DROP POLICY IF EXISTS "Users can view their own tokens" ON discord_tokens;
DROP POLICY IF EXISTS "Users can update their own tokens" ON discord_tokens;
DROP POLICY IF EXISTS "Admins can manage all tokens" ON discord_tokens;
DROP POLICY IF EXISTS "Users can view their own match history" ON match_history;
DROP POLICY IF EXISTS "Admins can manage all match history" ON match_history;

-- Create policies for discord_tokens
CREATE POLICY "discord_tokens_view_own"
ON discord_tokens
FOR SELECT
TO public
USING (auth.uid() = user_id);

CREATE POLICY "discord_tokens_update_own"
ON discord_tokens
FOR UPDATE
TO public
USING (auth.uid() = user_id);

CREATE POLICY "discord_tokens_admin_all"
ON discord_tokens
FOR ALL
TO service_role
USING (true)
...
CREATE POLICY "match_history_view_own"
ON match_history
FOR SELECT
TO public
USING (auth.uid() = user_id);

CREATE POLICY "match_history_admin_all"
ON match_history
FOR ALL
TO service_role
USING (true)
