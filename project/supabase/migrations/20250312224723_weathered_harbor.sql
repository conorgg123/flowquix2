...
DROP POLICY IF EXISTS "Users can view task recurrence" ON task_recurrence;
DROP POLICY IF EXISTS "Users can manage task recurrence" ON task_recurrence;
DROP POLICY IF EXISTS "Users can view task priorities" ON task_priorities;
...
-- Create task_recurrence table if it doesn't exist
...
-- Create task_priorities table if it doesn't exist
...
CREATE POLICY "task_dependencies_view"
...
CREATE POLICY "task_recurrence_view"
ON task_recurrence
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "task_recurrence_manage"
ON task_recurrence
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "task_priorities_view"
ON task_priorities
FOR SELECT
TO authenticated
USING (
